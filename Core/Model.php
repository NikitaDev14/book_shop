		...
		*/
		public function select_book($cond)
		{
			...
			else
			{
				...
				$id_book = $this->db->query($query);
			}
			
			$book = array("author" => array(),"genre" => array(),"name" => "temp","price" => "temp");
			$catalogue = array();
			
			$sql = 'SELECT GROUP_CONCAT(DISTINCT authors.Name) as Authors, GROUP_CONCAT(DISTINCT genres.Name) as Gesres, books.Name, books.Price
				FROM genres
				  JOIN genres2books
